...
def database_encounter(mydb, mycursor, encounterid, bossid, player, roleid, dps, hps, thps, aps, time, totaltime, date, bossname, playername, guildid, playerclass):
    new_encounterid = 0
    ...
    if myresult:
        ...
        print(date)
        sql = "INSERT INTO encounterinfo (guildid, encounterid, bossid, time, totaltime, date) VALUES (%s,%s, %s, %s, %s, %s)"
    ...
    myresult = mycursor.fetchall()
    if not myresult:
        sql = "INSERT INTO encounter (encounterid, playerid, roleid, dps, hps, thps, aps) VALUES (%s, %s, %s, %s, %s, %s, %s)"
...
def change_rolename(rolename):
    ...
    elif "tank" in rolename and "dps" in rolename and "heal" in rolename:
        rolename = "tank/dps/heal"
    elif rolename == "support/heal/dps" or rolename == "heal/dps/support":
        rolename = "dps/heal/support"
    elif rolename == "heal/support/tank" or rolename == "heal/dps/support":
        rolename = "tank/heal/support"
    elif rolename == "dps/tank/support" or rolename == "heal/dps/support":
        rolename = "tank/dps/support"
    elif rolename == "heal/tank/support" or rolename == "heal/dps/support":
        rolename = "tank/heal/support"
    return rolename
...
def main():
    ...
    if os.path.isfile("../help_files/dps.tsv"):
        ...
        for item in file:
            ...
            database_encounter(mydb, mycursor, encounterid, boss[bossname], player, role[rolename], dps, hps, thps, aps, time, totaltime, date, bossname, playername, guild[guildname], playerclass)
            # print(boss)
        file.close()
